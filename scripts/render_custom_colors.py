...
def id2rgb_custom(id):
    # Custom color mapping for Mcity Grayscale Segmentation
    # ID 0: Background -> Black
    # ID 127: Gray Region -> Blue
    # ID 255: White Region -> Red
    ...
    if id == 0:
        ...
        return np.array([0, 0, 255], dtype=np.uint8) # Blue
    elif id == 255:
        return np.array([255, 0, 0], dtype=np.uint8) # Red
    else:
        # Fallback for noise/other IDs: Random colors
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, classifier):
    ...
    pred_obj_path = os.path.join(model_path, name, "ours_{}".format(iteration), "objects_pred_custom")
    ...
    out_path = os.path.join(render_path[:-8],'concat_custom')
